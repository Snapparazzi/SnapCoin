...
interface Artifacts {
    ...
    require(name: './SNPCPreICO.sol'): IContract<ISNPCPreICO>;

    require(name: './SNPCICOStage1.sol'): IContract<ISNPCICOStage1>;

    require(name: './SNPCICOStage2.sol'): IContract<ISNPCICOStage2>;

    require(name: './SNPCICOStage3.sol'): IContract<ISNPCICOStage3>;
...
declare type TokenGroup = 'team' | 'bounty' | 'partners' | 'reserve' ;
 ...
 * ERC20 SNPC Token
 */
interface ISNPCToken extends IBaseICOToken {
    // Token name
    ...
    // Timestamp when partners and team reserved tokens will be unlocked
    reservedTeamUnlockAt: ISimpleCallable<NumberLike>;

    burnRemain(tr?: Web3.TransactionRequest): Promise<ITXResult>;
...
interface IBaseICO extends IContractInstance, IOwnable, IWhitelisted {
    ...
    // Number of sold tokens by this ICO
    tokensSold: ISimpleCallable<NumberLike>;
...
/**
 * SNPC token base contract
 */
interface ISNPCBaseICO extends IBaseICO {
    ...
    getEthTokenExchangeRatio: {
        call(tr?: Web3.TransactionRequest): Promise<NumberLike>;
    };

    transferTokens(investor: address, amount: NumberLike, tr?: Web3.TransactionRequest): Promise<ITXResult>;
}

/**
 * SNPC token Pre-ICO smart contract.
 */
interface ISNPCPreICO extends ISNPCBaseICO {
    ICO_STAGE: ISimpleCallable<number>;
}

/**
 * SNPC token ICO Stage 1 smart contract.
 */
interface ISNPCICOStage1 extends ISNPCBaseICO {
    ICO_STAGE: ISimpleCallable<number>;
}

/**
 * SNPC token ICO Stage 2 smart contract.
 */
interface ISNPCICOStage2 extends ISNPCBaseICO {
    ICO_STAGE: ISimpleCallable<number>;
}

/**
 * SNPC token ICO Stage 3 smart contract.
 */
interface ISNPCICOStage3 extends ISNPCBaseICO {
    ICO_STAGE: ISimpleCallable<number>;
}
